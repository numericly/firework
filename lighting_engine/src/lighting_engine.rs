pub mod lighting_engine {
    ...
    pub fn lighting_update_in_section(section_index: usize, sections: &mut [[Option<ChunkSection>; 26]; 25]) { //recalculates lighting in the 3x3x3 cube around the section updated
        ...
        for x in 0..80 {
            for y in 0..80 {
                for z in 0..80 {
                    ...
                    let lvl = get_source_brightness(x, y, z, &sections[(x / 16  + z / 16 * 5) as usize][(y / 16) as usize]);
                    if lvl != 0 {
        ...
        for x in 0..80 {
            for z in 0..80 {
                for y in (0..320).rev() {//loop through 319 to 0
                    if is_transparent(x%16, (y%16) as u8, z%16, &sections[(x / 16  + z / 16 * 5) as usize][(y / 16) as usize]) {
                        queue.push(QueuedLightingUpdate {
                            x,
        ...
        for update in queue {
            ...
            propagate_lighting(update.x, update.y, update.z, update.light_level, sections, update.is_sky_light);
        }

    }


    fn propagate_lighting(x: u8, y: u16, z: u8, lvl: u8, sections: &mut [[Option<ChunkSection>; 26]; 25], is_sky_light: bool) {
        if 
           get_source_brightness(x % 16, (y%16) as u8, z%16, &sections[(x / 16  + z / 16 * 5) as usize][(y / 16) as usize]) == 0   //if the block is not a light source
        ...
        || y>47
        || z>47
        || is_opaque(x%16, (y%16) as u8, z%16, &sections[(x / 16 + z / 16 * 5) as usize][(y / 16) as usize])                                 //or if the block is opaque
        { return }                                                                                                             //then do nothing

        //set light level of current block
        set_brightness(x%16, (y&16) as u8, z%16, sections[(x / 16 + z / 16 * 5) as usize][(y / 16) as usize].as_mut().unwrap(), lvl, is_sky_light);
    ...
    fn is_transparent(x: u8, y: u8, z: u8, section: &Option<ChunkSection>) -> bool {
        ...
        if section.is_none() || section.as_ref().unwrap().block_states.data.is_none() { println!("<ERROR> in lighting_engine.is_transparent, chunk or chunk data was none: section = {:?}", section); return false; }
        false
    }

    fn is_opaque(x: u8, y: u8, z: u8, section: &Option<ChunkSection>) -> bool {
        //x y z are coordinates within the section
        if section.is_none() || section.as_ref().unwrap().block_states.data.is_none() { println!("<ERROR> in lighting_engine.is_opaque, chunk or chunk data was none: section = {:?}", section); return true; }
    ...
    fn get_brightness(x: u8, y: u8, z: u8, section: &Option<ChunkSection>, is_sky_light: bool) -> u8 {
        if section.is_none() { return 0; }
        let lighting_data: &[i8; 2048]; //TODO: maybe make this use if let if that's relevant
        if is_sky_light {
            if section.as_ref().unwrap().sky_light.is_none() { return 0; }
            lighting_data = section.as_ref().unwrap().sky_light.as_ref().unwrap();
        }
        else {
            if section.as_ref().unwrap().block_light.is_none() { return 0; }
    ...
    fn set_brightness(x: u8, y: u8, z: u8, section: &mut ChunkSection, lvl: u8, is_sky_light: bool) {
        ...
        if x%2 == 0 {
            ...
            lighting_data[((x + y * 16 + z * 16 * 16) / 2) as usize] = (lighting_data[((x + y * 16 + z * 16 * 16) / 2) as usize] as u8 >> 4) as i8;//FIXME: does this copy the whole chunk section?
        } else {
            //uses the second four bits
            lighting_data[((x + y * 16 + z * 16 * 16) / 2) as usize] = (lighting_data[((x + y * 16 + z * 16 * 16) / 2) as usize] as u8 & 0b00001111) as i8;//FIXME: does this copy the whole chunk section?
        }
    }
    ...
    fn get_source_brightness(x: u8, y: u8, z: u8, section: &Option<ChunkSection>) -> u8 {
        ...
        if section.is_none() {//for unloaded chunk sections, return 0
            return 0;
        }
        0u8
    }

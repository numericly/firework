use protocol::client_bound::{LoginDisconnect, PlayDisconnect, Pong, ServerStatus};
...
impl Client {
    pub async fn handle(stream: TcpStream) -> Result<Option<Client>, ConnectionError> {
        let mut protocol = Protocol::new(stream);

        let handshake = read_packet_or_err!(Handshake, protocol, ConnectionState::HandShaking);

        protocol.connection_state = handshake.next_state;

        if handshake.next_state == ConnectionState::Status {
            Client::handle_ping(protocol).await?;
            return Ok(None);
        }

        if handshake.next_state != ConnectionState::Login {
            return Err(ConnectionError::UnexpectedNextState(handshake.next_state));
        }
        Ok(Some(Client { protocol }))
    }
    async fn handle_ping(protocol: Protocol) -> Result<(), ConnectionError> {
        let _status_request = read_packet_or_err!(StatusRequest, protocol, ConnectionState::Status);
        ...
        protocol.write_packet(server_status).await?;

        let ping_request = read_packet_or_err!(Ping, protocol, ConnectionState::Status);
        println!("<- Ping");
        let pong = Pong {
            payload: ping_request.payload,
        };
        protocol.write_packet(pong).await?;
        println!("-> Pong");
        return Ok(());
    }
}
